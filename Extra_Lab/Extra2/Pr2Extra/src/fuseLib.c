...
static int my_read(const char *path, char *buf, size_t size, off_t offset,   struct fuse_file_info *fi){
	...
	int bytes2Read = size;
	int indexFilenode=-1;
	if((indexFilenode = findFileByName(&myFileSystem, (char *)path + 1)) == -1) {
		return -ENOENT;
	}
	fi->fh = myFileSystem.directory.files[indexFilenode].nodeIdx;
	NodeStruct *node = myFileSystem.nodes[fi->fh];
	while(bytes2Read){
...
static int my_unlink(const char *path){
	...
	NodeStruct *node = myFileSystem.nodes[myFileSystem.directory.files[indexFilenode].nodeIdx];
	node->numLinks--;
	if(node->numLinks==0){
		int numBlocks = (BLOCK_SIZE_BYTES - 1) / BLOCK_SIZE_BYTES;
		myFileSystem.superBlock.numOfFreeBlocks += (node->numBlocks - numBlocks);
		int i;
		for(i = node->numBlocks; i > numBlocks; i--) {
			int nBloque = node->blocks[i - 1];
		...
		myFileSystem.directory.files[indexFilenode].freeFile=1;
		...
		updateNode(&myFileSystem, indexFilenode, node);
		free(node);
	}
	else{
		myFileSystem.directory.files[indexFilenode].freeFile=1;
...
static int my_mknod(const char *path, mode_t mode, dev_t device)
{
	...
	myFileSystem.nodes[idxNodoI]->numLinks=0;
