...
int readBitmap(MyFileSystem *myFileSystem)
{
	...
	if(read(myFileSystem->fdVirtualDisk,myFileSystem->bitMap,
...
int readDirectory(MyFileSystem* myFileSystem)
{
	...
	if(read(myFileSystem->fdVirtualDisk,&myFileSystem->directory, sizeof(DirectoryStruct)) == -1) {
		perror("Failed reaad in readDirectory");
		return -1;
	}
...
int readInodes(MyFileSystem* myFileSystem)
{
	...
	for(nodeNum=0;nodeNum< MAX_NODES;nodeNum++){
		...
		readNode(myFileSystem,nodeNum,node);
		if (!node->freeNode) {
			myFileSystem->numFreeNodes--;
			myFileSystem->nodes[nodeNum] = malloc(sizeof (NodeStruct));
			copyNode(myFileSystem->nodes[nodeNum], node);
		}
		else{
			myFileSystem->nodes[nodeNum] = NULL;
		}
		free(node);
	}
