...
#define RIGHT_LED_ON 0x2
#define RIGHT_LED_OFF 0x5
#define LEFT_LED_ON 0x4
#define LEFT_LED_OFF 0x3
                           ...
                           loff_t * offset)
{
  ...
  //  set_leds(kbd_driver,0x6);
    ...
    //kbd_driver= get_kbd_driver_handler();
    //set_leds(kbd_driver,LEFT_LED_ON);
...
device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
 ...
 //   int bytes_to_write = len;
    char copia[BUF_LEN+1];
    int i=0; 
    unsigned int boolN=0;
    unsigned int boolC=0;
   ...
   kbd_driver= get_kbd_driver_handler();
    ...
    if (copy_from_user(copia,buff,len))
        return -EFAULT;
   ...
   // copia[len] = '\0';
    ...
    for(i=0; i<len; i++){
        if(copia[i] == '1' && !boolN) { //NUM LOCK
            leds |= 0x2;
            boolN = 1;
        }        
            
        else if(copia[i] == '2' && !boolC)  {       //CAPS LOCK
            leds |= 0x4;
            boolC = 1;
        }
        else if(copia[i] == '3' && !boolS){         //CAPS LOCK
            leds |= 0x1;
            boolS = 1;
        }
    }
